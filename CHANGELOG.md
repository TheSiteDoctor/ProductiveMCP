...
- Four starter templates: `stripe-integration` (the sandbox-to-live Stripe sequence for Umbraco Commerce: per-environment keys and webhook signing secrets, end-to-end sandbox payment tests, client account activation, statement descriptor, wallet domain verification and a live smoke test), `umbraco-setup` (an add-on layering Umbraco/Igloo installation and client CMS training onto standard-delivery's task lists via same-name reuse), `standard-delivery` (TSD's standard Project Management / Infrastructure setup / Go-live Launch breakdown, parameterised by `domain_name`, including kick-offs, weekly status updates, budget/burn-rate review, RAID log, UAT coordination, close-down and a 30-day post-launch review) and `site-go-live` (the full 175-task go-live checklist, converted from the legacy JIRA-era checklist with its wiki markup translated to Markdown, browser/no-JS testing modernised, and the transactional email tasks made provider-neutral via an `email_provider` variable defaulting to Mailgun, with an SPF/DKIM DNS check added).
