...
| **Pages**    | Markdown         | JSON object (`{"type":"doc",...}`) | `markdownToProductiveDoc()` |

Pages use Productive's ProseMirror document format. The body attribute must be a **JSON object** (not a stringified JSON string). The `ProductiveDoc` type in `src/types.ts` defines the structure.
