...
Reusable ticket templates live as JSON files in `templates/` (overridable via `PRODUCTIVE_TEMPLATES_DIR`). `src/utils/templates.ts` loads/validates them and handles `{{variable}}` substitution; `src/tools/templates.ts` implements the list/get/apply tools. Templates follow TSD's Productive hierarchy: task list = project phase (`foundation_list`/`build_list`/`launch_list` variables), top-level ticket = Feature (epic), children = tasks. Applying a template creates task lists (reusing same-named ones, keeping phases in order) and nested tasks via `parent_task` relationships, reusing any ticket whose title already exists at the same level and adding only its missing children. Format reference: `docs/templates.md`. `npm run templates:report` builds an interactive HTML preview of all templates (`scripts/template-report.mjs`).
